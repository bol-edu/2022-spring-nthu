...
// ==============================================================
`timescale 1ns/1ps
module resize_accel_control_s_axi
#(parameter
    ...
    output wire [31:0]                   dst_rows,
    output wire [31:0]                   dst_cols,
...
localparam
    ...
    ADDR_DST_ROWS_DATA_0 = 6'h20,
    ADDR_DST_ROWS_CTRL   = 6'h24,
    ADDR_DST_COLS_DATA_0 = 6'h28,
    ADDR_DST_COLS_CTRL   = 6'h2c,
    WRIDLE               = 2'd0,
    WRDATA               = 2'd1,
    WRRESP               = 2'd2,
    WRRESET              = 2'd3,
...
//------------------------Local signal-------------------
    ...
    reg  [31:0]                   int_dst_rows = 'b0;
    reg  [31:0]                   int_dst_cols = 'b0;
    ...
    if (ACLK_EN) begin
        if (ar_hs) begin
            ...
            case (raddr)
                ...
                ADDR_DST_ROWS_DATA_0: begin
                    rdata <= int_dst_rows[31:0];
                end
                ADDR_DST_COLS_DATA_0: begin
                    rdata <= int_dst_cols[31:0];
                end
            endcase
        end
    end
...
assign dst_rows          = int_dst_rows;
assign dst_cols          = int_dst_cols;
// int_ap_start
always @(posedge ACLK) begin
    if (ARESET)
        int_ap_start <= 1'b0;
...
always @(posedge ACLK) begin
    if (ARESET)
        ...
        if (w_hs && waddr == ADDR_DST_ROWS_DATA_0)
            int_dst_rows[31:0] <= (WDATA[31:0] & wmask) | (int_dst_rows[31:0] & ~wmask);
    end
end
...
always @(posedge ACLK) begin
    if (ARESET)
        ...
        if (w_hs && waddr == ADDR_DST_COLS_DATA_0)
            int_dst_cols[31:0] <= (WDATA[31:0] & wmask) | (int_dst_cols[31:0] & ~wmask);
    end
end
