...
typedef struct {
    ...
    XResize_accel_uio_map maps[ MAX_UIO_MAPS ];
} XResize_accel_uio_info;

/***************** Variable Definitions **************************************/
static XResize_accel_uio_info uio_info;
...
static int uio_info_read_name(XResize_accel_uio_info* info) {
    char file[ MAX_UIO_PATH_SIZE ];
    sprintf(file, "/sys/class/uio/uio%d/name", info->uio_num);
    return line_from_file(file, info->name);
}

static int uio_info_read_version(XResize_accel_uio_info* info) {
    char file[ MAX_UIO_PATH_SIZE ];
    sprintf(file, "/sys/class/uio/uio%d/version", info->uio_num);
    return line_from_file(file, info->version);
}

static int uio_info_read_map_addr(XResize_accel_uio_info* info, int n) {
    int ret;
    char file[ MAX_UIO_PATH_SIZE ];
...
static int uio_info_read_map_size(XResize_accel_uio_info* info, int n) {
    int ret;
    char file[ MAX_UIO_PATH_SIZE ];
...
int XResize_accel_Initialize(XResize_accel *InstancePtr, const char* InstanceName) {
	XResize_accel_uio_info *InfoPtr = &uio_info;
