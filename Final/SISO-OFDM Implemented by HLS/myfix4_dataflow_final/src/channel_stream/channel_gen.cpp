...
#pragma HLS INTERFACE ap_fifo port=channel_out_imag
	...
	if(TAPS_NUM==2){
		for(i = 0; i < TAPS_NUM; i++){
			x_out_r = x_out_r + (ap_fixed<IN_WL,IN_IL>)(weight_2taps[i]*n[i]*sqrt_No/sqrt_2)*x_real[i];
			x_out_i = x_out_i + (ap_fixed<IN_WL,IN_IL>)(weight_2taps[i]*n[i]*sqrt_No/sqrt_2)*x_imag[i];
		}
	}
	else if(TAPS_NUM==3){
		for(i = 0; i < TAPS_NUM; i++){
			...
			//x_out_r = x_out_r + (ap_fixed<IN_WL,IN_IL>)((weight_3taps[i]*n[i]*sqrt_No/sqrt_2)*x_real[i]);
			//x_out_i = x_out_i + (ap_fixed<IN_WL,IN_IL>)((weight_3taps[i]*n[i]*sqrt_No/sqrt_2)*x_imag[i]);
		}
	}
	else if(TAPS_NUM==6){
		for(i = 0; i < TAPS_NUM; i++){
			x_out_r = x_out_r + (ap_fixed<IN_WL,IN_IL>)(weight_6taps[i]*n[i]*sqrt_No/sqrt_2)*x_real[i];
			x_out_i = x_out_i + (ap_fixed<IN_WL,IN_IL>)(weight_6taps[i]*n[i]*sqrt_No/sqrt_2)*x_imag[i];
		}
	}
	else{
		for(i = 0; i < TAPS_NUM; i++){
			x_out_r = x_out_r + (ap_fixed<IN_WL,IN_IL>)(weight_9taps[i]+n[i]*sqrt_No/sqrt_2)*x_real[i];
			x_out_i = x_out_i + (ap_fixed<IN_WL,IN_IL>)(weight_9taps[i]+n[i]*sqrt_No/sqrt_2)*x_imag[i];
		}
	}
	/*
	//AWGN generation
