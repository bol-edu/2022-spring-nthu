...
end entity adder_control_s_axi;
...
architecture behave of adder_control_s_axi is
    ...
    constant ADDR_INC_DATA_0   : INTEGER := 16#28#;
    constant ADDR_INC_CTRL     : INTEGER := 16#2c#;
    ...
    signal int_inc             : UNSIGNED(31 downto 0) := (others => '0');
        ...
        if (ACLK'event and ACLK = '1') then
            if (ACLK_EN = '1') then
                if (ar_hs = '1') then
                    ...
                    when ADDR_INC_DATA_0 =>
                        rdata_data <= RESIZE(int_inc(31 downto 0), 32);
...
-- ----------------------- Register logic ----------------
    ...
    begin
        if (ACLK'event and ACLK = '1') then
            if (ACLK_EN = '1') then
                if (w_hs = '1' and waddr = ADDR_INC_DATA_0) then
                    int_inc(31 downto 0) <= (UNSIGNED(WDATA(31 downto 0)) and wmask(31 downto 0)) or ((not wmask(31 downto 0)) and int_inc(31 downto 0));
                end if;
            end if;
        end if;
    end process;
