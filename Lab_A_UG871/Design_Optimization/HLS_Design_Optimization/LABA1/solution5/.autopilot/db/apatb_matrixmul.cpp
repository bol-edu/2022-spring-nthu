...
static bool RTLOutputCheckAndReplacement(std::string &AESL_token, std::string PortName) {
  ...
  return err;}
extern "C" void matrixmul_hw_stub_wrapper(volatile void *, volatile void *, volatile void *);
    ...
    string AESL_num;
{
      ...
      if (rtl_tv_out_file.good()) {
        ...
        if (atoi(AESL_num.c_str()) == AESL_transaction_pc) {
          std::vector<sc_bv<24> > b_pc_buffer(3);
          int i = 0;
          ...
          if (i > 0) {{
		    int i = 0;
            for (int j = 0, e = 3; j < e; j += 1, ++i) {((char*)__xlx_apatb_param_b)[j*4+0] = b_pc_buffer[i].range(7, 0).to_int64();
((char*)__xlx_apatb_param_b)[j*4+1] = b_pc_buffer[i].range(15, 8).to_int64();
((char*)__xlx_apatb_param_b)[j*4+2] = b_pc_buffer[i].range(23, 16).to_int64();
}}}
        } // end transaction
      } // end file is good
...
tr.reorder();
tr.send<2>((char*)__xlx_apatb_param_res, 9);
}
#else
{
      static ifstream rtl_tv_out_file;
...
tr.import<4>((char*)__xlx_apatb_param_a, 3, 0);
  }
tr.reorder();
...
sc_bv<24> __xlx_tmp_lv = ((int*)__xlx_apatb_param_a)[j];
...
aesl_fh.write(AUTOTB_TVIN_b, begin_str(AESL_transaction));
  if (__xlx_apatb_param_b) {
    for (int j = 0  - 0, e = 3 - 0; j != e; ++j) {
sc_bv<24> __xlx_tmp_lv = ((int*)__xlx_apatb_param_b)[j];
...
tr.import<2>((char*)__xlx_apatb_param_res, 9, 0);
  }
tr.reorder();
...
matrixmul_hw_stub_wrapper(__xlx_apatb_param_a, __xlx_apatb_param_b, __xlx_apatb_param_res);
...
aesl_fh.write(AUTOTB_TVOUT_b, begin_str(AESL_transaction));
  if (__xlx_apatb_param_b) {
    for (int j = 0  - 0, e = 3 - 0; j != e; ++j) {
sc_bv<24> __xlx_tmp_lv = ((int*)__xlx_apatb_param_b)[j];
aesl_fh.write(AUTOTB_TVOUT_b, __xlx_tmp_lv.to_string(SC_HEX)+string("\n"));
    }
  }
