...
extern "C" void matrixmul(int*, int*, short*);
extern "C" void apatb_matrixmul_hw(volatile void * __xlx_apatb_param_a, volatile void * __xlx_apatb_param_b, volatile void * __xlx_apatb_param_res) {
  ...
  vector<sc_bv<24> >__xlx_b__tmp_vec;
  for (int j = 0, e = 3; j != e; ++j) {
    sc_bv<24> _xlx_tmp_sc;
  ...
  for (int i = 0; i < __xlx_b__tmp_vec.size(); ++i) {
    __xlx_b__input_buffer[i] = __xlx_b__tmp_vec[i].range(23, 0).to_uint64();
  }
...
// print __xlx_apatb_param_b
  sc_bv<24>*__xlx_b_output_buffer = new sc_bv<24>[__xlx_size_param_b];
