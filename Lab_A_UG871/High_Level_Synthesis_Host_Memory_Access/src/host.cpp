...
struct job_type
{
    ...
    : context(c), queue(q), program(p), kernel_name(kname), nelements(nelements)
  {
    ...
    in1 = clCreateBuffer(context,CL_MEM_READ_ONLY,bytes,nullptr,&err);
    in2 = clCreateBuffer(context,CL_MEM_READ_ONLY,bytes,nullptr,&err);
    io = clCreateBuffer(context,CL_MEM_WRITE_ONLY,bytes,nullptr,&err);
